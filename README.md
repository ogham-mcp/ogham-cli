...
| **v0.6-alpha** | **Hybrid MCP proxy + wizard + schema polish.** `ogham serve` eager-spawns the Python sidecar and proxies every tool it exposes that isn't already native -- native handlers always win on name collision. Reconnect-on-death supervisor with 1s backoff + 15s spawn timeout recovers from Python crashes mid-session. Graceful degradation: `--no-sidecar` or a spawn failure drops to native-only. Auto-generated MCP tool JSON schemas via `github.com/invopop/jsonschema` (no hand-written JSON to drift). `ogham init` skips the API-key field for Ollama. `EMBEDDING_DIM` + provider URL env vars honoured (env wins over TOML). `SearchResult.Metadata` exposed natively (no side-join). Dim-agnostic hybrid search RPC (any halfvec dim works without migration). **v0.7 Batch A CRUD done (aside from switch/current_profile):** 7 more tools absorbed natively -- delete_memory, cleanup_expired, list_profiles, set_profile_ttl, reinforce_memory, contradict_memory, update_memory (re-embeds on content change via NewEmbedder). **Native-tool total: 11.** Python sidecar still owns switch/current_profile + typed-store wrappers + graph + compression. | Internal dogfood |
| v0.6 | Multi-language stopwords + extraction (de / fr / es / zh), recurrence extraction, narrower person-name regex. | Planned |
| v0.7 | switch_profile / current_profile (persistent active-profile state layer). Typed-store family (store_decision, store_event, store_fact, store_preference) + intent detection (reformulation / ordering / multi-hop / summary / temporal) + `record_access` on retrieved memories. | Planned |
