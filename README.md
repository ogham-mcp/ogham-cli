...
| `ogham triple <subject> <predicate> <object> [--profile P] [--fact-id UUID] [--metadata JSON]` | native (postgres) | **(v0.11)** Store a typed `(subject, predicate, object)` edge in the entity graph (mirrors the `store_triple` MCP tool). Subject/object resolve by canonical name or alias; predicate must be one of the 16 v1 vocab terms (`DEPENDS_ON`, `OWNS`, `ASSIGNED_TO`, `BLOCKS`, `PART_OF`, `SUPPORTS`, `RELATED_TO`, ...). Write-time supersession: a new edge for an existing `(subject, predicate, object, profile)` stamps the prior edge's `valid_to` and sets its `superseded_by`. `--sidecar` routes through the Python `store_triple` tool. |
| `ogham join <start-entity> --path P1,P2 --hop-limit N [--direction outgoing\|incoming] [--profile P]` | native (postgres) | **(v0.11)** Breadth-first traversal of the entity graph along a predicate path (mirrors `query_join`). `--hop-limit` is required (must be >= path length). Reads only current edges, terminates on cycles, and returns entities in BFS order plus the edges walked and their source-memory citations. An unresolvable start entity or a dead-end hop is an empty result, not an error. `--sidecar` routes through the Python `query_join` tool. |
